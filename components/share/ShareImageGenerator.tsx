...
export default function ShareImageGenerator({
  ...
  const shareToSocial = async (platform: 'twitter' | 'facebook' | 'instagram') => {
    ...
    const text = encodeURIComponent('나의 글로벌 자산 순위를 확인해보세요! #PayLens');
    const url = encodeURIComponent('https://paylens-kappa.vercel.app');
    ...
    switch (platform) {
      ...
      case 'facebook':
        ...
        break;
      case 'instagram':
        // 인스타그램은 직접 공유 불가, 이미지 다운로드 안내
        alert('이미지가 다운로드되었습니다. 인스타그램에서 업로드해주세요!');
        break;
    }
  };
  ...
  return (
    <div>
      ...
      <div
        ref={contentRef}
        className="fixed -left-[9999px] top-0 w-[1200px] h-[630px]"
        style={{
          transform: 'scale(1)',
